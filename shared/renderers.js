(function (root) {
  ...
  const api = {
    txt(data, options) {
      return header(data, options) + data.messages.map(m => `${role(m.role, data.provider)}:\n${m.text}`).join('\n\n' + '─'.repeat(48) + '\n\n') + '\n';
    },
    md(data, options) {
      return header(data, options, true) + data.messages.map(m => `## ${role(m.role, data.provider)}\n\n${m.markdown || m.text}`).join('\n\n---\n\n') + '\n';
    },
    ...
    html(data, options, printPage = false) {
      ...
      const messages = data.messages.map(m => `<article class="${m.role}"><h2>${role(m.role, data.provider)}</h2><div class="message">${m.html}</div></article>`).join('');
